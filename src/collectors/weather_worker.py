...
# Same hourly variables for Open-Meteo archive (scoring + historical validation).
OPEN_METEO_HOURLY_PARAMETERS: list[str] = [
    "temperature_2m",
    "precipitation",
...
def open_meteo_archive_query_params(
    lat: float,
    lon: float,
    start_date: str,
    end_date: str,
...
def features_from_open_meteo_hourly(hourly: dict[str, Any]) -> dict[str, float]:
    """Aggregate Open-Meteo hourly block into the same feature keys as WeatherWorker."""
...
class WeatherWorker:
    ...
    def collect(self, point: CandidatePoint) -> tuple[dict[str, float], dict[str, Any]]:
        params = open_meteo_archive_query_params(
            point.lat, point.lon, self.start_date, self.end_date
        )
